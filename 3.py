...
def extract_year(row): # РџРѕР»СѓС‡Р°РµС‚ РіРѕРґ РёР· С‚Р°Р№РјСЃС‚РµРјРїР°
    year = datetime.fromisoformat(row['published_timestamp'].replace('Z', '').replace('T',' ')).year
    row['published_timestamp'] = year
    return row

df = pd.read_csv('udemy_courses.csv')
df = df.apply(extract_year,axis=1) # РџРµСЂРµРІРѕРґРёС‚ С‚Р°Р№РјСЃС‚РµРјРї РІ РіРѕРґР° РІ С‚Р°Р±Р»РёС†Рµ
